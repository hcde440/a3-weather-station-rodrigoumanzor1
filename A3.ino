 ...
 * and our DHT22 sensor to obtain temperature, humidity, and heat index readings. These values
 * are then published to an MQTT server using the PubSubCLient library that allows us to send 
 * and receive messages from said server. The aforementioned temperature, humidity, and pressure 
 * values are also shown on our OLED display board. 
*/
...
void reconnect() {
  ...
  while (!mqtt.connected()) {
    ...
    if (mqtt.connect(mac, mqtt_user, mqtt_password)) { //<<---using MAC as client ID, always unique!!!
      ...
      mqtt.subscribe("Rodrigo/+"); //we are subscribing to 'theTopic' and all subtopics below that topic
...
void loop() {
 //-------------GET THE TEMPERATURE (DHT22)--------------//
  ...
  if (millis() - timerOne > 10000) {
    ...
    mqtt.publish("Rodrigo/A3", message);
    timerOne = millis();
  }
